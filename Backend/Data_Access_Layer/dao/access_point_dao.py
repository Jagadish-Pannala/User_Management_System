...
class AccessPointDAO:
    ...
    def get_access_point_by_path_and_method(
        ...
        ap = (
            ...
            .filter_by(endpoint_path=endpoint_path, method=method.upper())
            .first()
        )
        if ap:
    ...
    def get_access_point_by_path_and_method_without_regex_check(
        self, endpoint_path: str, method: str
    ) -> Optional[AccessPoint]:
        self.db.expire_all()
        ap = (
    ...
    def get_access_point_by_id(self, access_id: int) -> Optional[AccessPoint]:
        return (
            self.db.query(AccessPoint)
            .options(
                joinedload(AccessPoint.permission_mappings).joinedload(
                    AccessPointPermission.permission
                )
            )
    ...
    def get_access_point_by_uuid(self, access_uuid: str) -> Optional[AccessPoint]:
        return (
            ...
            .filter_by(access_uuid=access_uuid)
            .first()
        )
    ...
    def update_access_point_permission(
        self, access_id: int, permission_code: Optional[str]
    ) -> Optional[AccessPointPermission]:
        app = (
            self.db.query(AccessPointPermission).filter_by(access_id=access_id).first()
        )
        ...
        permission = (
            ...
            .filter_by(permission_code=permission_code)
            .first()
        )
        if not permission:
    ...
    def get_permission_code_by_access_id(self, access_id: int) -> Optional[str]:
        ...
        permission = (
            self.db.query(Permissions)
            .filter_by(permission_id=app.permission_id)
            .first()
        )
    ...
    def get_unmapped_access_points(self) -> List[AccessPoint]:
        ...
        return (
            self.db.query(AccessPoint)
            .filter(~AccessPoint.permission_mappings.any())
            .all()
        )
    ...
    def create_permission(
        self, permission_code: str, description: Optional[str] = None
    ) -> Permissions:
        permission = Permissions(
            permission_code=permission_code, description=description
        )
    ...
    def create_access_permission_mapping(
        self, access_id: int, permission_id: int, assigned_by: int
    ...
    def get_all_access_point_permission_ids(self) -> List[int]:
        access_points = (
            self.db.query(AccessPoint)
            .options(joinedload(AccessPoint.permission_mappings))
            .all()
        )
    ...
    def get_unmapped_permissions(self) -> List[Permissions]:
        return (
            self.db.query(Permissions).filter(~Permissions.access_mappings.any()).all()
        )
    ...
    def get_permissions_for_access_point_debug(self, access_id: int) -> List[str]:
        ...
        try:
            ...
            mappings = self.db.execute(
                mapping_query, {"access_id": access_id}
            ).fetchall()
    ...
    def debug_database_setup(self):
        ...
        try:
            ...
            access_points = self.db.execute(
                text("SELECT COUNT(*) as count FROM Access_Point")
            ).fetchone()
            print(f"   Access Points in DB: {access_points['count']}")

            # Check Permissions table
            permissions = self.db.execute(
                text("SELECT COUNT(*) as count FROM Permissions")
            ).fetchone()
            print(f"   Permissions in DB: {permissions['count']}")

            # Check mapping table
            mappings = self.db.execute(
                text("SELECT COUNT(*) as count FROM Access_Point_Permission_Mapping")
            ).fetchone()
            print(f"   Permission Mappings in DB: {mappings['count']}")

            # Show sample data
            print("\n   Sample Access Points:")
            sample_aps = self.db.execute(
                text("SELECT * FROM Access_Point LIMIT 3")
            ).fetchall()
            for ap in sample_aps:
                print(f"     {dict(ap)}")

            print("\n   Sample Permissions:")
            sample_perms = self.db.execute(
                text("SELECT * FROM Permissions LIMIT 5")
            ).fetchall()

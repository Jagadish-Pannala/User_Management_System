...
from ..auth.jwt_validator import validate_jwt_token
from Backend.Business_Layer.utils.redis_cache import get_access_point_from_cache
from .permission_utils import check_permission   # ✅ ADD THIS IMPORT
import inspect
import traceback
...
class JWTMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # ✅ FETCH DB SESSION SAFELY
            db = getattr(request.state, "db", None)

            # ✅ RUN PERMISSION CHECK HERE
            path = request.url.path
            method = request.method

            print(f"JWT Middleware - Checking permissions for {method} {path}")

            permission_result = check_permission(path, method, decoded_token, db_session=db)

            # If permission check returned 401/403 JSONResponse → STOP request
            if isinstance(permission_result, JSONResponse):
                print("🚫 Permission denied by middleware")

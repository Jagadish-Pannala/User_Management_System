...
def update_ap(
    ...
    return service.update(access_uuid, data,request=request,current_user=request.state.user)
...
def unmap_permission(
    ...
    return service.unmap_permission_both(access_uuid, permission_uuid,request=request,current_user= request.state.user)
